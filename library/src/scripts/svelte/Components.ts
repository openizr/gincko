...
export default {
  ...
  Options(field, onUserAction) {
    ...
    return ({
      ...
      events: {
        focus: componentProps.onFocus,
        change: (details: { newValue: string; }): void => onUserAction('input', field.path, details.newValue),
      },
    });
  },
  Textfield(field, onUserAction) {
    ...
    return ({
      ...
      events: {
        ...
        change: (details: { detail: { newValue: string; }; }): void => {
          onUserAction('input', field.path, details.detail.newValue);
        },
      },
    });
  },
  DatePicker(field, onUserAction) {
    ...
    return ({
      ...
      events: {
        ...
        change: (details: { value: string; }): void => {
          if (/^(\d{4})\/(\d{2})\/(\d{2})$/.test(details.value)) {
            const newDate = new Date(new Date(details.value).getTime() + 25 * 3600 * 1000);
  ...
  Textarea(field, onUserAction) {
    ...
    return ({
      ...
      events: {
        ...
        iconClick: componentProps.onIconClick,
        iconKeyDown: componentProps.onIconKeyDown,
        change: (details: { value: string; }): void => onUserAction('input', field.path, details.value),
      },
    });
  },
